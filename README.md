...
Dependencies can be installed using conda/mamba:
...
```bash
# Create database
python trnagraph.py preprocess makedb -g genome.fa -t trnascan.out -r gtrna.fa -m namemap.txt

# Trim reads
python trnagraph.py preprocess trim -i manifest.txt -o output

# Map reads
python trnagraph.py preprocess map -i manifest.txt -d database -o output
```
...
```bash
python trnagraph.py build -i metadata.tsv -o results_dir -d db_name
```
...
```bash
python trnagraph.py graph -i results_dir/data.h5ad -o figures -g all
```
