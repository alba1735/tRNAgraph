...
class visualizer():
    ...
    def overviewPlot(self, adata, umapgroup, output):
        ...
        for i in plot_list:
            ...
            # Plot the data
            if i[1] == cluster:
                sns.scatterplot(x=adata.obs[umap1][~hdbscan_annotated], y=adata.obs[umap2][~hdbscan_annotated], s=point_size, ax=axs[i[2],i[3]], color=np.array([(0.5,0.5,0.5)]), alpha=0.5, legend=False)
                sns.scatterplot(x=adata.obs[umap1][hdbscan_annotated], y=adata.obs[umap2][hdbscan_annotated], s=point_size, ax=axs[i[2],i[3]], hue=adata.obs[i[1]][hdbscan_annotated], palette=pal, legend=False)
            else:
    ...
    def clusterPlot(self, adata, umapgroup, clustgrp, output, numeric=False):
        ...
        if masking:
            sns.scatterplot(x=adata.obs[umap1][~mask], y=adata.obs[umap2][~mask], s=point_size, ax=axs, color=np.array([(0.5,0.5,0.5)]), alpha=0.5)
            sns.scatterplot(x=adata.obs[umap1][mask], y=adata.obs[umap2][mask], s=point_size, ax=axs, hue=adata.obs[clustgrp][mask], palette=pal)
        else:
