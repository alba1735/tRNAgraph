...
def test_an_ordinary_run_reports_no_grouping_problems(tmp_path):
    """A grouping check that ignored which graph types were requested would abort every run.
    The parameter checks are gated on the type that reads them actually being asked for."""
    h5ad = _write_adata(tmp_path / "obj.h5ad")
    result = runner.invoke(cli.app, [
        "--skip-env-check", "--skip-update-check", "graph",
    ...
    assert "problem with this command" not in result.output
