...
- ~~**Redundant standalone `tools split` command**: With split-BAM generation folded into `build --readlengthsplit`/`addsplit`, the separate `preprocess split`/`tools split` CLI command duplicated logic those commands already ran automatically, and its own placeholder `results_uN`/`graphs_uN` output was immediately discarded/redone by the real pipeline anyway.~~ Resolved: the standalone command has been removed; `BamSplitter` is now purely an internal helper invoked by `build`/`addsplit`. At the same time, the `u<N>`/`o<N>` split BAM files it produces are no longer persisted by default -- they're deleted once merged into the AnnData object, since only the resulting `results_uN`/`graphs_uN` output and the AnnData object itself need to be kept around. Pass `--savesplitbams` to `build`/`addsplit` to keep them on disk.
