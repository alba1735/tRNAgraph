...
> [!IMPORTANT]
> A cached split is reused only while every split BAM is **newer than the unsplit BAM it was cut from**. A split can only ever be written after its parent, so an older one means the parent has been replaced since — usually by a re-map or by `preprocess map --dedup` writing deduplicated BAMs over the originals — and the split now describes reads that are no longer there. The run aborts naming the offending files rather than counting them, because the two read sets look identical once summed. Pass `--overwritesplits` to re-cut them.
>
> `--overwritebams` and `--overwritesplits` are separate because they are separate operations: re-mapping needs the FASTQs, and those are frequently gone by the time a stale split is noticed.
...
Output is a scatter over the `trna` axis (the combined overview, plus one per contrast under `individual/`) and the `amino` and `iso` axes as paired bar charts under `individual/`. Colours come from `colors.decoupling` in `--style`, keyed by channel label.

Every figure also writes its own numbers as a TSV under a `results/` directory **sister to your `--output` directory**, mirroring the figure path — so `graphs/<config>/<type>/` pairs with `results/<config>/<type>/` and the table is found by the same route as the plot. Rows are ordered **most notable first**: features called in at least one channel, then by `decoupling` (distance from the diagonal) descending — the same order the labels on the figure use, so the file and the plot never disagree about what mattered. Columns are `rank`, the feature, `contrast`, `log2_`/`padj_`/`called_` per channel, `decoupling`, `min_padj` (the stronger of the two channels, for re-sorting by raw significance) and `tier`.
