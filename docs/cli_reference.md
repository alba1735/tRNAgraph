...
`trnagraph --version` (and the header of every command's log) also shows which channel this checkout is running: `stable` (the `main` branch), `beta` (the `dev` branch), or `nightly @ <commit hash>` (any other branch, or a detached HEAD not sitting exactly on a release tag -- which instead just shows that tag).
