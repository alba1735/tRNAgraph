#!/usr/bin/env python3

import numpy as np
import pandas as pd
import matplotlib.pyplot as plt
import seaborn as sns
import os

plt.rcParams['savefig.dpi'] = 300
plt.rcParams['pdf.fonttype'] = 42
plt.rcParams['ps.fonttype'] = 42

class visualizer:
    def __init__(self, stats_file, output_file):
        self.stats_file = stats_file
        self.output_file = output_file

    def plot(self):
        if not os.path.exists(self.stats_file):
            print(f"Stats file not found: {self.stats_file}")
            return

        try:
            df = pd.read_csv(self.stats_file)
        except Exception as e:
            print(f"Error reading stats file: {e}")
            return
        
        # Calculate Discarded
        # Ensure columns exist
        required_cols = ['Sample', 'Raw_Reads', 'Clean_Reads', 'Merged_Reads', 'Unmerged_Reads']
        if not all(col in df.columns for col in required_cols):
            print(f"Stats file missing required columns: {required_cols}")
            return

        df['Discarded'] = df['Raw_Reads'] - df['Clean_Reads']
        df['Merged'] = df['Merged_Reads']
        df['Unmerged'] = df['Unmerged_Reads']
        
        # Select columns for plotting
        plot_df = df[['Sample', 'Merged', 'Unmerged', 'Discarded']]
        
        # Melt
        df_melt = plot_df.melt(id_vars='Sample', var_name='ReadType', value_name='Count')
        
        # Filter out 0 counts
        df_melt = df_melt[df_melt['Count'] > 0]
        
        # Filter < 1% (replicating R script logic)
        # Calculate totals per sample
        sample_totals = df_melt.groupby('Sample')['Count'].transform('sum')
        df_melt = df_melt[df_melt['Count'] > sample_totals / 100]
        
        if df_melt.empty:
            print("No data to plot after filtering.")
            return

        # Pivot back for pandas plotting (stacked bar)
        df_pivot = df_melt.pivot(index='Sample', columns='ReadType', values='Count')
        
        # Reorder columns if needed: Merged, Unmerged, Discarded
        cols = [c for c in ['Merged', 'Unmerged', 'Discarded'] if c in df_pivot.columns]
        df_pivot = df_pivot[cols]
        
        # Calculate width based on number of samples
        # R script: width = 3 + .25*length(unique(countsmelt$variable))
        num_samples = len(df_pivot.index)
        width = 3 + 0.25 * num_samples
        height = 6 # Default height
        
        # Create plot
        fig, ax = plt.subplots(figsize=(width, height))
        
        # Plot stacked bars
        # Using pandas plot integration with matplotlib
        df_pivot.plot(kind='bar', stacked=True, ax=ax, width=0.8, edgecolor='black', linewidth=0.5)
        
        # Styling
        ax.set_xlabel("Sample", fontsize=15, fontweight='bold')
        ax.set_ylabel("Total Reads")
        ax.set_title("Read Types per Sample")
        
        # X-axis labels
        ax.set_xticklabels(df_pivot.index, rotation=90, fontsize=9, fontweight='bold')
        
        # Legend
        ax.legend(title="Read\nType", bbox_to_anchor=(1.05, 1), loc='upper left')
        
        # Theme adjustments to match R theme_bw() somewhat
        ax.set_facecolor('white')
        ax.grid(False)
        for spine in ax.spines.values():
            spine.set_visible(True)
            spine.set_color('black')

        # Adjust layout to prevent clipping
        plt.tight_layout()
        
        print(f"Saving plot to {self.output_file}")
        plt.savefig(self.output_file, bbox_inches='tight')
        plt.close()

if __name__ == '__main__':
    pass
