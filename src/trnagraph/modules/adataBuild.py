...
def _analysis_pipeline_phase_names(nosizefactors):
    '''
    ...
    path with nothing worth reporting progress on). "Analyzing counts"/"Analyzing unique counts"
    are only present when DESeq2 actually runs.
    '''
    phases = ["Counting Reads"]
    if not nosizefactors:
        phases.append("Analyzing counts")
    phases.append("Counting Read Types")
    if not nosizefactors:
        phases.append("Analyzing unique counts")
    phases.append("Generating Read Coverage plots")
    return phases
...
def _full_build_phase_names(analysis_args):
    ...
    '''
    nosizefactors = getattr(analysis_args, 'nosizefactors', False)
    vst_strategy = str(getattr(analysis_args, 'vst', 'vst')).lower()
    readlengthsplit = getattr(analysis_args, 'readlengthsplit', None)

    analysis_phases = _analysis_pipeline_phase_names(nosizefactors)
...
class AnalysisPipeline:
    def __init__(self, args, expname=None, phase_tracker=None, variant_label=None):
        ...
        self.phase_tracker = phase_tracker if phase_tracker is not None else toolsTG.PhaseTracker(
            phases=_analysis_pipeline_phase_names(self.nosizefactors), logger=self.logger, desc="Build",
        )
